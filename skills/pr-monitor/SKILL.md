---
name: pr-monitor
description: 現在のセッションに紐づく GitHub PR（または引数で指定した任意の PR）の状態変化を Monitor ツールで継続監視し、新規コメント・レビュー・CI・マージ状態の変化を通知する。ユーザーが PR を作業中のセッションでは常に有効化の候補になるスキル。ユーザーが明示的に「監視して」と言わなくても、push 直後・レビュー待ち・CI 待ちの文脈で起動を提案してよい。
---
...
```
command:     bash ${HOME}/.local/share/repos/github.com/kawaz/claude-pr-monitor/main/scripts/pr-monitor.sh <OWNER/REPO> <PR_NUMBER>
