...
- ステータス: Accepted
- 日付: 2026-05-25
- 関連: DR-0001 (改名 + スコープ拡大), DR-0003 (watch-workflow 常駐 Monitor), `hooks/`, `skills/`

## 文脈

`pr-monitor` の SessionStart hook は「PR を検出して Claude に skill 起動を促す」という指示を stdout に出す設計。`watch-workflow` の追加にあたり、PostToolUse hook で push を検出して同様に Monitor 起動を促す形を取りたい。
...
```
[gh-monitor] Monitor リストに `watch-workflow: kawaz/foo` が無ければ、
Monitor ツールで command=`watch-workflow.sh kawaz/foo`, persistent=true を起動せよ。
```
...
```json
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "[gh-monitor] Monitor リストに `watch-workflow: kawaz/foo` が無ければ、Monitor ツールで command=`watch-workflow.sh kawaz/foo`, persistent=true を起動せよ。"
  }
}
```
...
3. **重複防止は description マッチで成立する**
   - Monitor の description フィールドに `<feature>: <key>` 規約 (例: `watch-workflow: kawaz/foo`, `watch-pr: kawaz/foo#123`) を入れる
...
- DR-0003: `watch-workflow` の常駐 Monitor 設計 (description = `watch-workflow: <user/repo>`)
